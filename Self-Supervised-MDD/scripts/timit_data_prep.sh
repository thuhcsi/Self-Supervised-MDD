...
conf_dir=`pwd`/Self-Supervised-MDD/config
...
for x in train_timit; do
  ...
  for y in wrd phn; do
    ...
    if [ $y == phn ]; then 
        ...
        python Self-Supervised-MDD/scripts/normalize_phone.py --map $map_file --to $phoneme_map --src $prepare_dir/$x/${y}_text.tmp --tgt $prepare_dir/$x/${y}_text
